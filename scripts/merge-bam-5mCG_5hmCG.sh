...
#SBATCH --mem=16G
#SBATCH --error=merge-bam-5mCG_5hmCG.err
#SBATCH --output=merge-bam-5mCG_5hmCG.out
#SBATCH --job-name=merge-bam-5mCG_5hmCG
...
for pattern in "A19_jun" "A21_jun" "A25_jun" "N03_jun" "N07_jun" "N13_jun" ; do
    if [ ! -f data/samples-5mCG_5hmCG/${pattern}.bam ]; then
        $SAMTOOLS merge -@ ${CPUS} -o data/samples-5mCG_5hmCG/${pattern}.bam demux-5mCG_5hmCG/*${pattern}.bam
